...
const POLLS_FROM = '2026-09-04';


/* ── parser (ported from docs/index.html) ── */
const ALL_KEYS = ["Likud","Religious Zionism","Otzma Yehudit","Shas","UTJ","Yesh Atid","National Unity","Yisrael Beiteinu","The Democrats","Bennett 2026","Together","Yashar","Yesodot Yisrael","Joint List","Ra'am","Hadash-Ta'al","Balad","Reservists","Unity","Amcha Yisrael","Haredi Public"];
...
function headerKey(txt){ const t=txt.toLowerCase();
  ...
  if(/haredi public|\bharedim\b/.test(t)) return 'Haredi Public';
  if(/\bunity\b/.test(t)) return 'Unity';
  return null; }
function wikiPlain(s){ return s.replace(/<ref[^>]*\/>/g,'').replace(/<ref[^>]*>[\s\S]*?<\/ref>/g,'')
  .replace(/\[\[[^\]|]*\|([^\]]*)\]\]/g,'$1').replace(/\[\[([^\]]*)\]\]/g,'$1')
  .replace(/\{\{small\|([^}]*)\}\}/gi,'$1')
  /* {{font color|#158b74|<u>Yashar</u>|link=Yashar (political party)}} — the party name is
     the template's SECOND positional parameter, and the blanket {{...}} strip below used to
     delete the whole thing. The header then read as empty, headerKey got nothing to match,
     and the column was dropped without even being logged as unrecognised, taking every
     column after it one place to the left. Six of the largest parties are written this way
     in the current table — Together, Yisrael Beiteinu, Ra'am, Joint List, Dems, Yashar —
     which is what produced the 10 Sep 2026 row that summed to 97 seats. Lift the visible
     text out before anything strips the template. */
  .replace(/\{\{\s*font\s*color\s*\|[^|}]*\|([^|}]*)(?:\|[^}]*)?\}\}/gi,'$1')
  .replace(/'''/g,'').replace(/''/g,'')
...
function parseWikiText(wikitext){
  const results=[]; const seen=new Set(); const rejected=[];
  ...
  while((ti=sec.indexOf('{|',ti))>=0){
    ...
    for(const ch of chunks){ if(/\{\{\s*Opdrts/i.test(ch)) continue;
      ...
      for(const hc of hcells){ const plain=wikiPlain(headerContent(hc)); const key=headerKey(plain);
        if(key==='JOINT2'){ const csm=hc.match(/colspan\s*=\s*"?(\d+)"?/i); const cs=csm?parseInt(csm[1],10):1;
          ...
          /* CONSUME the column even though we cannot name it. Pushing nothing here was the
             real fault behind the Yossi Tatika row of 10 Sep 2026: a new "Haredi Public"
             column appeared, headerKey could not place it, the column was skipped rather
             than held, and every column after it slid one place left. The warning below has
             always said such a party is "silently dropped" — it was worse than that, because
             a dropped column takes its neighbours with it. A placeholder keeps the row
             aligned, so an unknown party costs exactly that one party. */
          order.push('?' + plain);
    ...
    for(const ch of chunks){ if(!/\{\{\s*Opdrts/i.test(ch)) continue;
      ...
      cols.forEach((c,i)=>{ const v=convSeat(seatCells[i]);
        if(c[0]==='?') return;                                     // a column we hold open but cannot name
      ...
      /* Every Israeli poll apportions exactly 120 seats. The old 95..122 window was wide
         enough to wave through both a row that summed to 122 and one that summed to 97 —
         each of them a column-shift artefact rather than a poll — and once accepted they
         moved every average on the page, because pollBlocs() derives "others" as
         120 − gov − opp and simply absorbs the discrepancy. Take 120 and say what was
         rejected, so a parser fault surfaces as a missing poll rather than a wrong one. */
      if(gv===undefined) continue;
      if(total!==120){ rejected.push(`${iso} ${firm}: seats sum to ${total}, not 120`); continue; }
      rec._govTotal=gv; seen.add(dk); results.push(rec); }
    /* Only the top table. Older tables further down the page are frozen snapshots of
       earlier ballots — different columns, parties that have since merged or withdrawn —
       and mixing their rows in means the dataset describes two different elections at
       once. From the point the lists closed, the top table's shape is fixed through
       polling day, so one table is all there is to read. */
    break;
  }
  if(rejected.length) console.warn('\n\u26a0\ufe0f  Rejected %d row(s) whose seats did not sum to 120 (a column-shift signature):\n   %s', rejected.length, rejected.join('\n   '));
  return results;
}
